...
- [#20](https://github.com/nerima-games/mx-redstone/pull/20) [`d50aa71`](https://github.com/nerima-games/mx-redstone/commit/d50aa711689a614bd1bc41a0343e81965175312a) Thanks [@takeokunn](https://github.com/takeokunn)! - Renamed `domain/timed-power-graph.ts` and `test/timed-power-graph.test.ts` to `domain/power-timing.ts` and `test/power-timing.test.ts` — no behaviour change, just the evidence path the cross-repo feature catalog's `redstone/repeater-and-sticky-piston` row has always declared for repeater delay and button pulse timing.
